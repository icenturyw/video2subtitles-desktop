...
class WhisperApiClient:
    ...
    def transcribe_video(self, video_path, language=None, service="local", api_key=None):
        ...
        try:
            r = self.session.post(f"{self.base_url}/transcribe", json=payload, timeout=10)
            if r.status_code == 200:
                return r.json()
            return {"error": r.text}
    ...
    def transcribe_url(self, video_url, language=None, service="local", api_key=None):
        ...
        if api_key:
            payload["api_key"] = api_key
        try:
            r = self.session.post(f"{self.base_url}/transcribe", json=payload, timeout=30)
            if r.status_code == 200:
                return r.json()
            ...
            return {"error": "无法连接到服务器，请检查服务器是否运行"}
        except Exception as e:
